    ...
    <div className="mb-5 space-y-3">
      {ctx.phone && (
        <a
          ...
          className="flex items-center justify-between rounded-card bg-bee-charcoal px-5 py-4 text-white shadow-sm transition-transform hover:scale-[1.01] active:scale-[0.98]"
        >
          <div>
      ...
      {ctx.intro && ctx.step_label === "Step 1" && (
        <div className="rounded-card border border-bee-border bg-cream-dark/50 px-4 py-3 text-sm text-bee-ink lg:text-base">
          <p className="text-xs font-bold uppercase text-bee-muted">Opening line</p>
          <p className="mt-1 font-medium">{ctx.intro}</p>
        </div>
      )}

      <div className="rounded-card border-2 border-bee-yellow/40 bg-white p-5 lg:p-6">
        <p className="text-xs font-bold uppercase tracking-wide text-bee-muted">Say this</p>
        <p className="mt-2 text-lg font-semibold leading-relaxed text-bee-charcoal lg:text-xl">
      ...
      {ctx.rep_questions.length > 0 && (
        <div className="rounded-card border border-bee-border bg-bee-yellow-soft/50 p-4 lg:p-5">
          <p className="mb-3 text-sm font-bold text-bee-charcoal">If the rep asks…</p>
          <dl className="space-y-3">
            {ctx.rep_questions.map((q, i) => (
              <div key={i} className="rounded-card border border-bee-border bg-white px-4 py-3">
  ...
  return (
    <div className="mb-5 space-y-3">
      {enrichment.general_path && (
        <p className="rounded-card border border-bee-yellow/40 bg-bee-yellow-soft/60 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-bee-charcoal">
          General rollover guide — look for Withdrawals / Rollovers in your provider portal
        </p>
      )}

      <div className="rounded-card border-2 border-bee-yellow/40 bg-white p-5 lg:p-6">
        <p className="text-xs font-bold uppercase tracking-wide text-bee-muted">Do this now</p>
        <p className="mt-2 text-lg font-semibold leading-relaxed text-bee-charcoal lg:text-xl">
          {ctx.say_this}
        </p>
      </div>
      ...
      {ctx.portal_menu_hints && ctx.portal_menu_hints.length > 0 && (
        <div className="rounded-card border border-bee-border bg-cream-dark/50 px-4 py-3">
...
function FormsWalkthrough({
  ...
  return (
    <div className="mb-5 space-y-3">
      <div className="rounded-card border-2 border-dashed border-bee-yellow/50 bg-white p-5 lg:p-6">
        <p className="text-xs font-bold uppercase text-bee-muted">Form field</p>
        <p className="mt-1 text-lg font-bold text-bee-charcoal">{ctx.form_field_label}</p>
