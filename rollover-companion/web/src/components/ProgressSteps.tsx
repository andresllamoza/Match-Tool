          ...
          return (
            <div key={step.id} className="flex flex-1 flex-col items-center gap-2">
              <div className="flex w-full items-center">
                {i > 0 && (
                  <div
                    className={`h-0.5 flex-1 ${done || active ? "bg-bee-blue" : "bg-bee-border"}`}
                  />
                )}
                <div
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-colors lg:h-10 lg:w-10 lg:text-sm ${
                      ...
                      ? "bg-bee-blue text-white"
                      : active
                        ? "bg-bee-blue text-white ring-4 ring-bee-blue/20"
                        : "bg-cream-dark text-bee-muted border border-bee-border"
                  }`}
                >
                  {done ? "✓" : i + 1}
                </div>
                {i < STEPS.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 ${done ? "bg-bee-blue" : "bg-bee-border"}`}
                  />
                )}
              </div>
              <span
                className={`text-xs font-semibold lg:text-sm ${
                  active ? "text-bee-blue" : "text-bee-muted"
                }`}
              >
                {step.label}
              </span>
